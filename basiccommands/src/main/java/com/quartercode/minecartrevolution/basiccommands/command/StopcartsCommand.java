...
public class StopcartsCommand extends MRCommandHandler {
    ...
    public CommandInfo createInfo() {
        ...
        return new CommandInfo(true, "[-w World1,World2...] [-r Radius]", MinecartRevolution.getLang().get("basiccommands.stopcarts.description"), "minecartrevolution.command.stopcarts", "stopcarts");
    }
    ...
    private void stop(World world, int radius, Location location, CommandSender commandSender, String sender) {
        ...
        commandSender.sendMessage(MinecartRevolution.getLang().get("basiccommands.stopcarts.stopped", "count", String.valueOf(count), "world", world.getName(), "radius", radius < 0 ? "unlimited" : String.valueOf(radius)));
        Bukkit.broadcastMessage(MinecartRevolution.getLang().get("basiccommands.stopcarts.broadcast", "sender", sender, "count", String.valueOf(count), "world", world.getName(), "radius", radius < 0 ? "unlimited" : String.valueOf(radius)));
    }
